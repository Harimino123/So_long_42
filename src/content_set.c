...
void	draw_image_at_position(t_game *data, char map_char, int x, int y)
{
	...
	if (map_char == '1')
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.img_wall, img_x, img_y);
	else if (map_char == 'P')
	{
		data->pos.x = x;
		data->pos.y = y;
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.img_player, img_x, img_y);
	}
	else if (map_char == 'C')
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.img_collect, img_x, img_y);
	else if (map_char == 'E')
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.img_exit, img_x, img_y);
	else
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
			data->img.img_floor, img_x, img_y);
}
